...
namespace ps2_stubs {
    // FlushFrames stub - Force transition on first call to trigger level load
    void FlushFrames_stub(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        static int call_count = 0;
        static bool transition_forced = false;
        call_count++;

        // Force clear g_mpeg.oid_1 to escape video loop
        *reinterpret_cast<uint32_t*>(rdram + 0x169A00) = 0;

        // Read current g_transition and g_psw state
        uint32_t m_fPending = *reinterpret_cast<uint32_t*>(rdram + 0x275af0);
        uint32_t g_psw_val = *reinterpret_cast<uint32_t*>(rdram + 0x275710);

        if (call_count <= 10) {
            std::cout << "[FlushFrames #" << call_count << "] m_fPending=" << m_fPending
                     << " g_psw=0x" << std::hex << g_psw_val << std::dec << std::endl;
        }

        // Force transition if g_psw is NULL and transition not already pending
        if (!transition_forced && g_psw_val == 0 && m_fPending == 0) {
            std::cout << "[FlushFrames] FORCING TRANSITION!" << std::endl;
            std::cout << "  Setting m_fPending = 1" << std::endl;
            std::cout << "  Setting m_pchzWorld = 0x247ab0 (first level)" << std::endl;

            *reinterpret_cast<uint32_t*>(rdram + 0x275af0) = 1;  // m_fPending = true
            *reinterpret_cast<uint32_t*>(rdram + 0x275af4) = 0x247ab0;  // m_pchzWorld = first level
            *reinterpret_cast<uint32_t*>(rdram + 0x275af8) = 0xFFFFFFFF;  // m_oidWarp = OID_Nil
            *reinterpret_cast<uint32_t*>(rdram + 0x275afc) = 0xFFFFFFFF;  // m_oidWarpContext = OID_Nil
            *reinterpret_cast<uint32_t*>(rdram + 0x275b00) = 0;  // grftrans = FTRANS_None
