...
static const char* getSyscallName(int num) { /* DEBUG_INJECT */
    switch(num) { /* DEBUG_INJECT */
        ...
        case 0x3c: return "InitMainThread"; /* DEBUG_INJECT */ // Was verkeerd: SetupThread
        case 0x3d: return "InitHeap"; /* DEBUG_INJECT */       // Was verkeerd: SetupHeap
...
bool PS2Runtime::loadELF(const std::string &elfPath) {
    ...
    std::cout << "ELF has " << header.phnum << " program headers, entsize=" << header.phentsize << std::endl;
    for (uint16_t i = 0; i < header.phnum; i++) {
        ProgramHeader ph;
        file.seekg(header.phoff + i * header.phentsize);
        file.read(reinterpret_cast<char *>(&ph), sizeof(ph));
        std::cout << "  Segment " << i << ": type=" << ph.type << " vaddr=0x" << std::hex << ph.vaddr
                  << " filesz=0x" << ph.filesz << " memsz=0x" << ph.memsz << std::dec << std::endl;
